  ...
  if (reply) {
    ...
    if (reply.text) {
      bot.telegram.sendMessage(
        ...
        reply.text +
          "\n" +
          (reply.chat.id === parseInt(process.env.adminchat, 10)
            ? process.env.channel
    ...
    const caption =
      reply.chat.id === parseInt(process.env.adminchat, 10)
        ...
        : "By " + mes.from.first_name + "\n" + process.env.channel;
    if (reply.photo) {
      bot.telegram.sendPhoto(process.env.channel, reply.photo[0].file_id, {
        caption: caption,
      });
    } else if (reply.audio) {
      bot.telegram.sendAudio(process.env.channel, reply.audio.file_id, {
        caption: caption,
      });
    } else if (reply.video) {
      bot.telegram.sendVideo(process.env.channel, reply.video.file_id, {
        caption: caption,
      });
    }
  }
...
async function start() {
  try {
    ...
    bot.telegram.setWebhook(
      process.env.ADRESS + secretPath
    );
    app.get("/", (req, res) => res.send("Hello World!"));
    // Set the bot API endpoint
    app.use(bot.webhookCallback(secretPath));
    app.listen(process.env.PORT, () =>
      console.log(`Has started ${process.env.PORT}`)
    );
  } catch (e) {
